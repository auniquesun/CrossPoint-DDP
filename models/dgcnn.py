...
class DGCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((x1, x2, x3, x4), dim=1)  # Concat features, 512 = 64 + 64 + 128 + 256
        ...
        if self.cls != -1:  # do not use self-supervised pre-training, instead, train DGCNN directly for point cloud recognition
        ...
        else:               # self-supervised pre-training
            feat = x    # --- output feat ---: (batch_size, feature_dim=args.emb_dims*2), used for contrastive learning
            inv_feat = self.inv_head(feat)      # --- output feat ---: (batch_size, feature_dim=256), the features used for downstream tasks
...
class DGCNN_partseg(nn.Module):
    ...
    def forward(self, x, l = None):
        ...
        if self.pretrain:
            # print("Pretrain")
            x = x.squeeze()               # x: (batch_size, emb_dims)
            inv_feat = self.inv_head(x)
            
            return inv_feat, x              # inv_feat: (batch_size, 256)
        
        else:   # directly train partseg model rather than pre-training
            # print('Entering DGCNN_partseg, do not pretrain, instead finetune it !')
            #   here `l` is passed in through `forward` function
