...
def train(rank):
    ...
    print('---seg_num_all---:', seg_num_all)
    print('---seg_start_index---:', seg_start_index)
    ...
    # here args.model_path is save in DDP, so we can load dierectly in DDP model
    # reference: https://discuss.pytorch.org/t/missing-keys-unexpected-keys-in-state-dict-when-loading-self-trained-model/22379/9
    pretrained_model = torch.load(args.model_path, map_location=map_location)
    model_ddp.load_state_dict(
        pretrained_model, 
        strict=False    # becuase partseg head is different from that in Pre-Training, set strict=False
    )
    ...
    for epoch in range(args.epochs):
        ...
        """
        `train_loader` iterates on the tuple (data, label, seg)
            data:  (batch_size, num_points, 3), a point cloud has `num_points`=2048 points
            label: (batch_size, 1), object category for each point cloud
            seg:   (batch_size, num_points), semantic category for each point
        """
        ...
        if test_ious_avg >= best_test_iou:
            ...
            torch.save(model_ddp.module.state_dict(), 'outputs/%s/models/model.t7' % args.exp_name)
